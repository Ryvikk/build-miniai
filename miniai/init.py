...
# %% auto 0
__all__ = ['clean_ipython_hist', 'clean_tb', 'clean_mem', 'GeneralRelu', 'plot_func', 'init_weights', 'lsuv_init', 'conv',
           'get_model', 'BatchTransformCB', 'LSUVCallback']
...
class LSUVCallback(HooksCallback):
    ...
    def before_fit(self, learn):
        super().before_fit()
        for m, h in zip(self.mods, self.hooks): self.lsuv_init(m, h, learn)
        self.wi_done = True
    
    def lsuv_init(self, m_in, h, learn):
        with torch.no_grad():
            while learn.model(self.xb) is not None and (abs(h.std**2-1)>1e-3 or abs(h.mean)>1e-3):
                print('initializing... ', h.mean, h.std)
                m_in.bias -= h.mean
                m_in.weight.data /= h.std
            print('final: ', h.mean, h.std)
